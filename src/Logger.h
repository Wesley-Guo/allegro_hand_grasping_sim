...
public:
	// ctor
	Logger(long interval, std::string fname, const bool date_and_time_in_filename = true, const double realtime_scaling_factor = 1.0)
	: _log_interval_(interval)
	{
		// create log file
		_logfile.open(fname + "_" + currentDateTime() + ".csv", std::ios::out);
		_logfile << "timestamp, ";
